...
public class DeviceHandleFunction
{
    ...
    public static Map<String, List<ExpDataObject>> getDeviceEnergy(DbOperBase dbOper)
    {
        ...
        try
        {
            ...
            ResultSet ds = dbOper.getResultSet("select id,expdescr,sort,typeid from config.energyuse_config where  isenabled = 1 order by typeid,sort ", null);
            while (ds.next())
            {
            ...
            ResultSet dt = dbOper.getResultSet("select aa.*,bb.typeid,bb.sort from config.energyuse_exp aa join config.energyuse_config  bb on aa.id=bb.id", null);
            while (dt.next())
            {
                ExpDataObject obj = new ExpDataObject(dt.getInt("id"), dt.getString("expdescr"), dt.getInt("sort"),ds.getInt("typeid"));
